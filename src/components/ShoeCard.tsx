...
export default function ShoeCard({ shoe, selected = false, onToggle }: Props) {
  return (
    <div
      className={`bg-white rounded-2xl p-5 flex justify-between items-start gap-4 border transition ${
      ...
      <div className="min-w-0">
        ...
        {onToggle ? (
          ...
          <div className="flex items-center gap-2">
            <button className="font-display font-bold text-sm bg-corail text-white px-3.5 py-2 rounded-lg hover:opacity-90 transition">
              Comparer
            </button>
            <span className="font-display font-extrabold text-xs tracking-wide bg-encre text-volt px-3 py-1.5 rounded-full">
              VS
            </span>
          </div>
        )}
